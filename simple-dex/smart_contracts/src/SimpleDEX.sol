...
contract SimpleDEX {
    ...
    function swapDogeToArbi(uint256 dogeAmount) external {
        uint256 arbiAmount = dogeAmount / rate;
        // Integer division truncates toward zero, so a dogeAmount smaller than
        // `rate` would otherwise let the DEX take the user's dogeFake while
        // paying out zero arbiFake in return.
        require(arbiAmount > 0, "Amount too small to swap");

        require(arbiFake.balanceOf(address(this)) >= arbiAmount, "Not enough arbiFake in DEX");
